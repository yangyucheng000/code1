...
class REDS():
    ...
    def build(
        ...
        Args:
            ...
            drop_remainder: A boolean indicating whether to drop remained elements.
                Defaults to True. When false the output of dataset.batch() is None in the batch_size dimension. 
    ...
    def _preprocess(self, image_paths, label_paths):
        ...
        """
        image_tensor, label_tensor = tf.numpy_function(
            _loader, (image_paths, label_paths), (tf.float32, tf.float32))

        image_tensor.set_shape([image_paths.shape[0], None, None, 3])
        label_tensor.set_shape([label_paths.shape[0], None, None, 3])
        if self.split == 'val':
            image_tensor.set_shape([image_paths.shape[0], 180, 320, 3])
            label_tensor.set_shape([label_paths.shape[0], 720, 1280, 3])            
