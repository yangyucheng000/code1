...
def c2net_multidataset_to_env(multi_data_url, data_dir):
    ...
    for i in range(len(multi_data_json)):
        ...
        try:
            ...
            file_extension = os.path.splitext(multi_data_json[i]["dataset_name"])[1]
            if file_extension == '.zip':
                file_name = os.path.splitext(multi_data_json[i]["dataset_name"])[0]
                ...
                filePath = data_dir + "/" + base_name
                if not os.path.exists(filePath):
                    os.makedirs(filePath)
                os.system("tar -xzvf {} -C {}".format(zipfile_path, filePath))
