...
def accuracy_torch(pred, target, topk=1, thrs=None):
    ...
    target_label = target.view(1, -1).expand_as(pred_label)
    if pred_label.dtype != target_label.dtype:
        target_label = target_label.astype(pred_label.dtype)

    correct = pred_label.equal(target_label)
    ...
    for k in topk:
        ...
        for thr in thrs:
            ...
            correct_k = _correct[:k].reshape(-1).float().sum(0, keepdims=True)
            res_thr.append(mindspore.ops.mul(correct_k, 100.0 / num))
        if res_single:
            res.append(res_thr[0])
        else:
            res.append(res_thr)
