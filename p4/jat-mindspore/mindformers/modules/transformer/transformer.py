...
class MultiHeadAttention(Cell):
    ...
    def construct(self, query_tensor, key_tensor, value_tensor, attention_mask, key_past=None,
        ...
        jat_score = None
        if self.j_attn is not None:
            jat_score = self.j_attn(query, key.swapaxes(2, 3))
        if not self.training and self.use_prompt_flash_attention:
            if self.use_past and not self.is_first_iteration:
                if self.use_incre_flash_attention:
                    ...
                    attention = self._attn(query, key, value, attention_mask, jat_score, self.j_ceof)
            else:
                query, key, attention_mask = self._pfa_ifa_data_preprocess(query, key, attention_mask,
                                                                           batch_valid_length)
    ...
    def _attn(self, query, key, value, attention_mask, j_attn=None, ceof=0.5):
        """
        ...
        if j_attn is not None:
            _, hd, _ , _ = j_attn.shape
            attention_scores[:,0:hd,:,:] = attention_scores[:,0:hd,:,:] * (1 - ceof) + j_attn * ceof
...
class TransformerEncoderLayer(Cell):
    ...
    def __init__(self,
        ...
        if _get_parallel_mode() in (ParallelMode.AUTO_PARALLEL,):
            ...
            attention_parallel_config = parallel_config.dpmp if self.use_moe else parallel_config
            self.jat_attention = None
            if self.layer in cfg.olayers and cfg.jat_heads > 0:
                self.jat_attention = JatAttn(cfg, softmax_compute_type)
            self.attention = MultiHeadAttention(batch_size=batch_size,
                                                src_seq_length=seq_length,
                                                tgt_seq_length=seq_length,
                                                hidden_size=hidden_size,
