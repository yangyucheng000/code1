...
class AutoFE:
    ...
    def set_cuda(self, cuda):
        ...
        if cuda == 'False':
            ...
            # self.is_cuda = mindspore.cuda.is_available()
            print("GPU used")
            mindspore.set_context(device_target="GPU", device_id=0)
            # if self.is_cuda:
            #     logging.info(f"Use device: {cuda}, {self.device}, {mindspore.cuda.get_device_name(self.device)}")
            #     return
        logging.info(f"Use device: {self.device}")
        print("device set finish")
    ...
    def fit_attention(self, args):
        ...
        init_state_c = Tensor(df_c_encode.values).float().swapaxes(0, 1)
        init_state_d = Tensor(df_d_encode.values).float().swapaxes(0, 1)
                ...
                for step in range(args.steps_num):
                    ...
                    for i in range(args.episodes):
                        ...
                        if df_c_encode.shape[0] > 1:
                            ...
                            workers_c[i].m1.append(mindspore.tensor(m1_output.asnumpy()))
                            workers_c[i].m2.append(mindspore.tensor(m2_output.asnumpy()))
                            workers_c[i].m3.append(mindspore.tensor(m3_output.asnumpy()))
                            workers_c[i].action_softmax.append(mindspore.tensor(action_softmax.asnumpy()))
                        if args.combine:
                            actions, log_probs, m1_output, m2_output, m3_output, action_softmax = self.ppo.choose_action_d(
                                workers_d[i].states[-1], step,
        ...
        df_combined.to_csv('/result/output.csv')
