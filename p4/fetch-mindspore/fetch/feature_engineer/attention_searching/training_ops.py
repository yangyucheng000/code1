...
def sample(args, ppo, pipline_args_train, df_c_encode, df_d_encode, df_t_norm, c_ops, d_ops, epoch, i, device):
    ...
    # init_state_c = mindspore.Tensor.from_numpy(df_c_encode.values).float().transpose(0, 1)
    init_state_c = mindspore.Tensor.from_numpy(df_c_encode.values).float().swapaxes(0, 1)
    # init_state_d = mindspore.Tensor.from_numpy(df_d_encode.values).float().transpose(0, 1)
    init_state_d = mindspore.Tensor.from_numpy(df_d_encode.values).float().swapaxes(0, 1)
    ...
    for step in range(steps_num):
        ...
        if df_c_encode.shape[0] > 1:
            ...
            if x_c_encode.shape[0]:
                ...
                # x_encode_c = mindspore.Tensor.from_numpy(x_encode_c).float().transpose(0, 1)
                x_encode_c = mindspore.Tensor.from_numpy(x_encode_c).float().swapaxes(0, 1)
        ...
        if args.combine:
            ...
            try:
                ...
                breakpoint()
            # x_encode_d = mindspore.Tensor.from_numpy(x_encode_d).float().transpose(0, 1)
            x_encode_d = mindspore.Tensor.from_numpy(x_encode_d).float().swapaxes(0, 1)
