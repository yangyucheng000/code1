...
class PPO(object):
    def __init__(self, args, data_nums, operations_c, operations_d, d_model, d_k, d_v, d_ff, n_heads,
        ...
        if operations_c:
            ...
            self.actor_c_opt = optim.Adam(params=self.actor_c.get_parameters(), lr=args.lr)
        if operations_d:
            ...
            self.actor_d = Actor(args, data_nums, operations_d, d_model, d_k, d_v, d_ff, n_heads, dropout=dropout, enc_load_pth=args.enc_d_pth)
            self.actor_d_opt = optim.Adam(params=self.actor_d.get_parameters(), lr=args.lr)
        if c_param:
            self.actor_c.load_state_dict(mindspore.load(c_param)["net"])
            self.actor_c_opt.load_state_dict(mindspore.load(c_param)["opt"])
    ...
    def choose_action_c(self, input_c, step, epoch, ops, sample_rule):
        ...
        self.actor_c.set_train()
        
        # print(type(input_c))
        # print(type(step))
        
        action_softmax, m1_output, m2_output, m3_output = self.actor_c(input=input_c, step=step)
        ...
        for index, out in enumerate(action_softmax):
            ...
            if index in index_none:
                ...
                action = mindspore.tensor(len(ops) - 1)
            else:
                action = dist.sample()
    ...
    def choose_action_d(self, input_d, step, epoch, ops, sample_rule):
        actions = []
        log_probs = []

        self.actor_d.set_train()
        action_softmax, m1_output, m2_output, m3_output = self.actor_d(input_d, step)
        index_none = []
        for index, out in enumerate(action_softmax):
            dist = Categorical(out)
            if index in index_none:
    ...
    def update_c(self, workers_c):
        ...
        baseline_step = mindspore.tensor(baseline_step)
        ...
        rewards_convert = mindspore.tensor(rewards_convert).reshape(-1, self.args.steps_num)
        ...
        for worker_index, worker in enumerate(workers_c):
            for i, x in enumerate(worker.log_probs):
                for j, item in enumerate(x):
                    ...
                    workers_c[worker_index].log_probs[i][j] = mindspore.tensor(item)
            for i, x in enumerate(worker.actions):
                for j, item in enumerate(x):
                    # workers_c[worker_index].actions[i][j] = torch.tensor(item, device=self.device)
                    workers_c[worker_index].actions[i][j] = mindspore.tensor(item)
            for index, state in enumerate(worker.states):
                workers_c[worker_index].states[index] = state
        ...
        for epoch in range(self.args.ppo_epochs):
            ...
            def net_forward():
                ...
                for worker_index, worker in enumerate(workers_c):
                    ...
                    # print(f"worker_index: {worker_index}, advantage in update_c: {advantage}")
                    advantage_convert = []
                    ...
                    # advantage_convert = mindspore.tensor(mindspore.ops.stack(advantage_convert))
                    advantage_convert = mindspore.ops.stack(advantage_convert)

                    # old_log_probs = torch.tensor([item for x in old_log_probs_ for item in x], device=self.device)
                    old_log_probs = mindspore.ops.stack([item for x in old_log_probs_ for item in x])
                    ...
                    for index, state in enumerate(states):
                        ...
                        step = steps[index]
                        action_softmax, m1_output, m2_output, m3_output = self.actor_c(state, step)
                    ...
                    new_log_probs = mindspore.ops.cat(new_log_probs)
                    # entropys = torch.cat(entropys)
                    entropys = mindspore.ops.cat(entropys)
                    ...
                    weighted_clipped_probs = mindspore.ops.clamp(prob_ratio, 1 - self.clip_epsion,
                                                         1 + self.clip_epsion) * advantage_convert
                    # print("---------------------")
                    # print(weighted_probs)
                    # print(type(weighted_probs))
                    # print(weighted_clipped_probs)
                    # print(type(weighted_clipped_probs))
                    # print("---------------------")
                    # actor_loss = -torch.min(weighted_probs, weighted_clipped_probs)
                    # actor_loss = -mindspore.ops.min(weighted_probs, weighted_clipped_probs)
                    actor_loss = mindspore.ops.minimum(weighted_probs, weighted_clipped_probs)
                    actor_loss = mindspore.ops.neg(actor_loss)
                    # print("c actorloss done")
            ...
            net_backward = mindspore.value_and_grad(net_forward, None, self.actor_c_opt.parameters)
            loss, grads = net_backward()
            self.actor_c_opt(grads)
...
#             logging.info(
#                 f"total_loss_c:{total_loss.item()},actor_loss:{actor_loss.item()},entory_loss:{entropy_loss.item()}")
#             # self.actor_c_opt.zero_grad() mindspore cancel
#             # total_loss.backward()
    ...
    def update_d(self, workers_d):
        ...
        for worker_index, worker in enumerate(workers_d):
            for i, x in enumerate(worker.log_probs):
                for j, item in enumerate(x):
                    # workers_c[worker_index].log_probs[i][j] = torch.tensor(item, device=self.device)
                    workers_d[worker_index].log_probs[i][j] = mindspore.tensor(item)
            for i, x in enumerate(worker.actions):
                for j, item in enumerate(x):
                    # workers_c[worker_index].actions[i][j] = torch.tensor(item, device=self.device)
                    workers_d[worker_index].actions[i][j] = mindspore.tensor(item)
            for index, state in enumerate(worker.states):
                workers_d[worker_index].states[index] = state
        ...
        for epoch in range(self.args.ppo_epochs):
            def net_forward():
                ...
                for worker_index, worker in enumerate(workers_d):
                    ...
                    for index, state in enumerate(states):
                        action = actions[index]
                        step = steps[index]
                        action_softmax, m1_output, m2_output, m3_output = self.actor_d(state, step)
                        for k, out in enumerate(action_softmax):
                            dist = Categorical(out)
                            entropy = dist.entropy()
                            entropys.append(entropy.unsqueeze(dim=0))
            ...
            net_backward = mindspore.value_and_grad(net_forward, None, self.actor_d_opt.parameters)
            loss, grads = net_backward()
            self.actor_d_opt(grads)
...
#                 old_log_probs = mindspore.tensor([item for x in old_log_probs_ for item in x])
...
#                         new_log_prob = dist.log_prob(mindspore.tensor(action[k])).unsqueeze(dim=0).float()
#                         new_log_probs.append(new_log_prob)
...
#             logging.info(
#                 f"total_loss_d:{total_loss.item()},actor_loss:{actor_loss.item()},entory_loss:{entropy_loss.item()}")
#             # self.actor_d_opt.zero_grad() mindspore cancel
#             # total_loss.backward()
#             self.actor_d_opt.step()
    ...
    def save_model_c(self):
        ...
        if not os.path.exists(dir):
            os.makedirs(dir)
        mindspore.save({"net": self.actor_c.state_dict(), "opt": self.actor_c_opt.state_dict()},
