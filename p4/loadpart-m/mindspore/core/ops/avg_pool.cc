...
abstract::ShapePtr InferShape(const PrimitivePtr &primitive, const std::vector<AbstractBasePtr> &input_args) {
  ...
  auto format = Format(CheckAndConvertUtils::GetAndCheckFormat(primitive->GetAttr("format")));
  ...
  CheckAndConvertUtils::GetPadModEnumValue(primitive->GetAttr("pad_mode"), &pad_mode_value, true);
...
TypePtr InferType(const std::vector<AbstractBasePtr> &input_args) { 
  ...
  if (input_tensor_type == nullptr) {
    MS_LOG_EXCEPTION << "The avgpool's input must be a tensor but got " << input_type->ToString();
  }
