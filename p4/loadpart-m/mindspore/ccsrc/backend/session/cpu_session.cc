...
GraphId CPUSession::CompileGraphImpl(NotNull<FuncGraphPtr> func_graph) {
  // auto graph_id = graph_sum_;
  std::vector<KernelGraphPtr> all_graphs;
  auto graph = ConstructKernelGraph(func_graph, &all_graphs, DeviceAddressType::kCPU);
  // draw::Draw("KernelGraph.dot", graph);
  MS_EXCEPTION_IF_NULL(graph);
  if (all_graphs.size() != 1) {
    MS_LOG(EXCEPTION) << "Cpu backend does not support multi-graph schedule, graph num is " << all_graphs.size();
  }
  MS_EXCEPTION_IF_NULL(graph);
  UnifyMindIR(graph);
...
void CPUSession::SetRuntimeResourceForFakeGraph(KernelGraphPtr target) { 
  runtime_.SetRuntimeResourceForFakeGraph(target);
}

GraphId CPUSession::CompileGraphWithInferShape(NotNull<FuncGraphPtr> func_graph) {
  // auto graph_id = graph_sum_;
  std::vector<KernelGraphPtr> all_graphs;
  auto graph = ConstructKernelGraph(func_graph, &all_graphs, DeviceAddressType::kCPU);
  ...
  for (auto &node : node_list) {
    if (node->isa<CNode>()) {
      ...
      AnfAlgo::InferShape(cnode, nullptr);
    }
  }
