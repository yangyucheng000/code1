...
void GPUSession::SetRuntimeResourceForFakeGraph(KernelGraphPtr target) {
  auto runtime_instance = device::KernelRuntimeManager::Instance().GetSingleKernelRuntime(kGPUDevice, device_id_);
  MS_EXCEPTION_IF_NULL(runtime_instance);
  auto gpu_runtime_instance = dynamic_cast<device::gpu::GPUKernelRuntime *>(runtime_instance);
  gpu_runtime_instance->SetRuntimeResourceForFakeGraph(target);
}
...
GraphId GPUSession::CompileGraphWithInferShape(NotNull<FuncGraphPtr> func_graph) {
  std::vector<KernelGraphPtr> all_graphs;
  auto root_graph = ConstructKernelGraph(func_graph, &all_graphs, DeviceAddressType::kGPU);
  MS_EXCEPTION_IF_NULL(root_graph);
  if (all_graphs.size() != 1) {
  ...
  auto &node_list = root_graph->execution_order();
  for (auto &node : node_list) {
    // std::cout << "Processing " << node->fullname_with_scope() << std::endl;
    if (node->isa<CNode>()) {
