...
void GpuInferenceSession::LoadInputData(const std::shared_ptr<KernelGraph> &kernel_graph,
  ...
  for (size_t i = 0; i < input_nodes.size(); ++i) {
    ...
    if (!AnfAlgo::IsParameterWeight(pk_node)) {
      ...
      //   std::cout << "LoadInputData: [" << i << "] = " << pk_node->fullname_with_scope() << " : " << tensor->data_c() << " -> " << tensor->device_address() << " -> " << tensor->device_address()->GetMutablePtr() << ", s: " << tensor->sync_status() << " need_sync: " << need_sync << std::endl;
      // } else {
      //   std::cout << "LoadInputData: [" << i << "] = " << pk_node->fullname_with_scope() << " , s: " << tensor->sync_status() << " need_sync: " << need_sync << std::endl;
      // }
      if (need_sync) {
...
GraphId GpuInferenceSession::CompileGraphWithInferShape(NotNull<FuncGraphPtr> func_graph) {
  auto graph_id = GPUSession::CompileGraphWithInferShape(func_graph);
  auto kernel_graph = GetGraph(graph_id);
  MS_EXCEPTION_IF_NULL(kernel_graph);
  // load weight data to device
  auto input_nodes = kernel_graph->inputs();
