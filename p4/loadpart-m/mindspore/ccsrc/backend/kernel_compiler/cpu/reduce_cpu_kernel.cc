...
void ReduceCPUKernel<T>::InitKernel(const CNodePtr &kernel_node) {
  ...
  // special accelerate for axis = (1, ..., dim), dim >= 1 and input_shape[0] = 1
  if (axis_.size() == input_shape_.size() - 1) {
    int64_t d = 1;
    bool check_axis = true;
    for (auto a : axis_) {
      check_axis &= (a == d++);
    }
    fast_execute_ = check_axis & (input_shape_[0] == 1);
  } else {
    fast_execute_ = false;
  }
}
...
bool ReduceCPUKernel<T>::Launch(const std::vector<kernel::AddressPtr> &inputs, const std::vector<kernel::AddressPtr> &,
  ...
  if (axis_.empty() || input_shape_.empty() || input_shape_.size() == 1 || fast_execute_) {
