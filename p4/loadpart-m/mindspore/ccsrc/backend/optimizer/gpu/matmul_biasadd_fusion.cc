...
const AnfNodePtr MatMulBiasAddFusion::Process(const FuncGraphPtr &graph, const AnfNodePtr &node,
  ...
  const AnfNodePtr &weight_load = AnfAlgo::GetInputNode(utils::cast<CNodePtr>(matmul), 1);
  MS_EXCEPTION_IF_NULL(weight_load);
  const AnfNodePtr &bias_load = AnfAlgo::GetInputNode(utils::cast<CNodePtr>(node), 1);
  MS_EXCEPTION_IF_NULL(bias_load);
  ...
  std::vector<AnfNodePtr> inputs = {prim_value, x_input, weight_load, bias_load};
  ...
  if (kg) {
    // kg->FrontBackendlMapUpdate(weight_load, load_w);
    // kg->FrontBackendlMapUpdate(bias_load, load_bias);
    kg->FrontBackendlMapUpdate(matmul, fused_node);
    kg->FrontBackendlMapUpdate(node, fused_node);
  }
