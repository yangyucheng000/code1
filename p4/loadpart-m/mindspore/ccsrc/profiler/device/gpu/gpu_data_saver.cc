...
void GpuDataSaver::WriteKernelEvent(const std::string &saver_base_dir, const std::vector<Event> &events) {
  std::string file_path = saver_base_dir + "/gpu_kernel_event_info.csv";
  std::ofstream ofs(file_path);
  // check if the file is writable
  if (!ofs.is_open()) {
    MS_LOG(WARNING) << "Open file '" << file_path << "' failed!";
  ...
  // TODO: Separate events by devices if needed

  // header
  ofs << "kernel_name,kernel_type,start_time_stamp,duration,op_name,device_id,stream_id" << std::endl;
  for (const auto &event : events) {
    if (event.op_name.empty() || event.api_type != CUPTIApiType::kActivity ||
        event.activity_type != ActivityType::kKernel) {
      continue;
    }
    ofs << "\"" << event.kernel_name << "\","
        << event.kernel_type << ","
        << event.start_time_stamp << ","
        << (event.end_time_stamp - event.start_time_stamp) / kTimeUnit << ","
        << event.op_name << ","
        << event.device_id << ","
        << event.stream_id << std::endl;
  }

  ofs.close();
  ChangeFileMode(file_path);
  MS_LOG(INFO) << "Write gpu kernel event infos into file: " << file_path;
}
}  // namespace gpu
}  // namespace profiler
}  // namespace mindspore
