...
void GPUProfiler::StopWithoutWriteFile(std::unordered_map<std::string, float>& result) {
  MS_LOG(INFO) << "Stop GPU Profiling without saving files";
  StopCUPTI();
  OpsParser();
  ...
  dataSaver.GetAverageOpTime(result);
  ClearInst();
}
