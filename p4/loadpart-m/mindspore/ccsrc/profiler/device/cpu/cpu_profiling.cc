...
void CPUProfiler::StopWithoutWriteFile(std::unordered_map<std::string, float>& result) {
  MS_LOG(INFO) << "Stop CPU Profiling without saving files";
  ...
  cpu_data_saver_inst->GetAverageOpTime(result);
  for (auto &it : result) {
    it.second *= 1000;
  }
  ClearInst();
