...
void CPUSimpleMemPlan::MemAssign(const session::KernelGraph *graph, uint8_t *base_ptr) {
  ...
  for (const auto &kernel : kernels) {
    ...
    for (size_t i = 0; i < input_num; ++i) {
      ...
      MS_EXCEPTION_IF_NULL(address);
      // std::cout << "MemAssign for " << kernel->fullname_with_scope() << " input " << kernel_with_index.first->fullname_with_scope() << "[" << i << "]" << ": " << address << " -> " << address->ptr_ << std::endl;
      if (address->ptr_ == nullptr) {
    ...
    for (size_t i = 0; i < output_num; ++i) {
      ...
      MS_EXCEPTION_IF_NULL(address);
      // std::cout << "MemAssign for " << kernel->fullname_with_scope() << " output [" << i << "]" << ": " << address << " -> " << address->ptr_ << std::endl;
      if (address->ptr_ == nullptr) {
        address->ptr_ = mem_ptr;
        mem_ptr = mem_ptr + address->size_;
      }
    ...
    for (size_t i = 0; i < kernel_mod->GetWorkspaceSizeList().size(); ++i) {
      auto address = AnfAlgo::GetWorkspaceAddr(kernel, i);
      MS_EXCEPTION_IF_NULL(address);
      // std::cout << "MemAssign for " << kernel->fullname_with_scope() << " wkspace [" << i << "]" << ": " << address << " -> " << address->ptr_ << std::endl;
      if (address->ptr_ == nullptr) {
        address->ptr_ = mem_ptr;
        mem_ptr = mem_ptr + address->size_;
      }
