...
void CPUKernelRuntime::SetRuntimeResourceForFakeGraph(KernelGraphPtr target) {
  MS_EXCEPTION_IF_NULL(target);
  target->SetExecOrderByDefault();
  AssignKernelAddress(target.get());
}
...
bool CPUKernelRuntime::Run(const session::KernelGraph &kernel_graph, bool) {
  ...
  for (const auto &kernel : kernels) {
    ...
    // std::cout << "Execute " << kernel->fullname_with_scope() << ": " << std::endl;
    for (size_t i = 0; i < input_num; ++i) {
      ...
      // std::cout << "Input [" << i << "]: " << device_address << " -> " << device_address->ptr() << std::endl;
    }
    ...
    for (size_t i = 0; i < output_num; ++i) {
      ...
      // std::cout << "Output [" << i << "]: " << device_address << " -> " << device_address->ptr() << std::endl;
    }
    auto kernel_mod = AnfAlgo::GetKernelMod(kernel);
    MS_EXCEPTION_IF_NULL(kernel_mod);
    for (size_t i = 0; i < kernel_mod->GetWorkspaceSizeList().size(); ++i) {
      ...
      // std::cout << "Workspace [" << i << "]: " << device_address << " -> " << device_address->ptr() << std::endl;
    }
    bool ret = true;
#ifndef ENABLE_SECURITY
