...
bool GPUDeviceAddress::SyncDeviceToHost(size_t size, void *host_ptr) const {
  ...
  // std::cout << "SyncDeviceToHost: " << ptr_ << " -> " << host_ptr << " " << size << "B" << std::endl; 
...
bool GPUDeviceAddress::SyncHostToDevice(size_t size, const void *host_ptr) const {
  ...
  // std::cout << "SyncHostToDevice: " << host_ptr << " -> " << ptr_ << " " << size << "B" << std::endl; 
...
bool GPUDeviceAddress::AsyncDeviceToDevice(const DeviceSync *src_device_addr, size_t offset) const {
  MS_EXCEPTION_IF_NULL(src_device_addr);
  auto src_gpu_device = dynamic_cast<const GPUDeviceAddress *>(src_device_addr);
  MS_EXCEPTION_IF_NULL(src_gpu_device);
  auto src_size = src_gpu_device->GetSize();
  ...
  if (src_size + offset > size_) {
    MS_LOG(ERROR) << "The src device size is greater than the dst + offset device size, src device size: " << src_size
                  << ", dst device size: " << size_;
    return false;
  }
  ...
  auto &stream = GPUDeviceManager::GetInstance().default_stream();
  MS_EXCEPTION_IF_NULL(stream);
  auto actual_ptr = reinterpret_cast<char *>(ptr_) + offset;
  if (!GPUDeviceManager::GetInstance().CopyDeviceMemToDeviceAsync(reinterpret_cast<void *>(actual_ptr), src_ptr, src_size, stream)) {
    MS_LOG(ERROR) << "CopyDeviceMemToDeviceAsync failed";
    return false;
  }
  return true;
