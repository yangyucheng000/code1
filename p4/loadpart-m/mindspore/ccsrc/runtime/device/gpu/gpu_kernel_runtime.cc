...
void GPUKernelRuntime::SetRuntimeResourceForFakeGraph(KernelGraphPtr target) {
...
#endif
    // std::cout << "Executing " << kernel->fullname_with_scope() << std::endl; 
    auto kernel_mod = AnfAlgo::GetKernelMod(kernel);
    MS_EXCEPTION_IF_NULL(kernel_mod);
...
bool GPUKernelRuntime::AllocKernelInputDynamicRes(const mindspore::AnfNodePtr &kernel, AddressPtrList *kernel_inputs,
  ...
  MS_EXCEPTION_IF_NULL(mem_reuse_util_);
  size_t input_num = AnfAlgo::GetInputTensorNum(kernel);
  // std::cout << kernel->fullname_with_scope() << std::endl;
  for (size_t i = 0; i < input_num; ++i) {
    ...
    // std::cout << "Input[" << i << "] at " << device_address << " -> " << device_address->ptr_ << " of " << input->size << "B" << std::endl;
...
bool GPUKernelRuntime::AllocKernelOutputDynamicRes(const mindspore::kernel::KernelMod &kernel_mod,
  ...
  for (size_t i = 0; i < output_sizes.size(); ++i) {
    ...
    // std::cout << "Output[" << i << "] at " << device_address << " -> " << device_address->ptr_ << " of " << output->size << "B" << std::endl;
...
bool GPUKernelRuntime::AllocKernelWorkspaceDynamicRes(const mindspore::kernel::KernelMod &kernel_mod,
  ...
  for (size_t i = 0; i < workspace_sizes.size(); ++i) {
    ...
    // std::cout << "WorkSpace[" << i << "] at " << device_address << " -> " << device_address->ptr_ << " of " << workspace->size << "B" << std::endl;
